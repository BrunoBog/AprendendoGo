...
import (
	...
	"github.com/AprendendoGo/Defer/model"
)
...
func main() {
	...
	defer arquivo.Close() // Defer é um comandao que serve apra informar o sistema operacional que o recurso deve ser fechado quando o programa terminar
	...
	for _, linha := range conteudo {
		for indiceItem, item := range linha {
			...
			cidade := model.Cidade{} //model.Cidade{}
			cidade.Nome = dados[0]
